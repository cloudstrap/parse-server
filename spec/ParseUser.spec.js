...
describe('Parse.User testing', () => {
        ...
        Parse.User.signUp('asdf', 'zxcv', null, {
            success: () => {
                return rp
                    ...
                    .then(res => {
                        fail(
                            `no request should succeed: ${JSON.stringify(res)}`
                        );
                        done();
                    })
                    .catch(err => {
    ...
    it('user loaded from localStorage from signup', done => {
        Parse.User.signUp('alice', 'password', null, {
            success: function(alice) {
                ...
                ok(
                    alice.getSessionToken(),
                    'Alice should have a session token'
                );
                equal(
                    alice.get('password'),
                    undefined,
                ...
                equal(
                    aliceAgain.id,
                    alice.id,
                    'currentUser should have objectId'
                );
                ok(
                    aliceAgain.getSessionToken(),
            ...
            .then(
                function() {
                    ...
                    equal(
                        userFromDisk.id,
                        id,
                        'id should be set on userFromDisk'
                    );
        ...
        Parse.User.signUp('bob', 'welcome', null, {
            success: function(user) {
                ok(
                    ...
                    'Subclassing User should have worked'
                );
            ...
            success: function(user) {
                ...
                user.save('secret', 1337, {
                    success: function() {
                        ...
                        ok(
                            userFromDisk.isSuper(),
                            'The subclass should have been used'
                        );
                        done();
                    },
                    error: function() {
        ...
        Parse.User.signUp(
            ...
            expectSuccess({
                success: function(user) {
                    ...
                    ok(
                        !userAgain.dirty('style'),
                        'The user was just read from disk.'
                    );
                    done();
                }
            })
    ...
    it('log in with provider and update token', done => {
        var provider = getMockFacebookProvider();
        var secondProvider = getMockFacebookProviderWithIdToken(
            '8675309',
            'jenny_valid_token'
        );
            ...
            success: () => {
                ...
                return Parse.User.logOut().then(() => {
                    Parse.User._logInWith('facebook', {
                        success: () => {
                            expect(
                                secondProvider.synchronizedAuthToken
                            ).toEqual('jenny_valid_token');
            ...
            success: function(model) {
                ...
                Parse.User._logInWith('facebook', {
                    success: function(innerModel) {
                        ok(
                            innerModel instanceof Parse.User,
                        ...
                        ok(
                            provider.authData.id === provider.synchronizedUserId
                        );
                        ok(
                            provider.authData.access_token ===
                                provider.synchronizedAuthToken
                        ...
                        ok(
                            innerModel._isLinked('facebook'),
                            'User should be linked to facebook'
                        );
                        ok(
                            innerModel.existed(),
                            'User should not be newly-created'
                        );
                        done();
                    },
                    error: function(model, error) {
        ...
        user.signUp(null, {
            success: function() {
                user._linkWith('facebook', {
                    success: function(model) {
                        ok(
                            model instanceof Parse.User,
                            'Model should be a Parse.User'
                        );
                        strictEqual(Parse.User.current(), model);
                        strictEqual(
                            provider.authData.id,
                            provider.synchronizedUserId
                        );
                        strictEqual(
                            provider.authData.access_token,
                            provider.synchronizedAuthToken
            ...
            success: function() {
                user._linkWith('facebook', {
                    success: function(model) {
                        ...
                        user2.set(
                            'username',
                            'testLinkWithProviderToAlreadyLinkedUser2'
                        );
                        ...
                        request.put(
                            {
                                ...
                                url:
                                    'http://localhost:8378/1/sessions/' +
                                    b.objectId,
        ...
        Parse.User.signUp('asdf', 'zxcv', null, {
            success: function() {
                request.get(
                    ...
                    (error, response, body) => {
                        expect(body.results[0].expiresAt.__type).toEqual(
                            'Date'
                        );
                        done();
                    }
            ...
            .then(user => {
                ...
                return new Promise((resolve, reject) => {
                    request.put(
                        {
                            url:
                                'http://localhost:8378/1/classes/_User/' +
                                user.objectId,
                            headers: {
                                'X-Parse-Application-Id': Parse.applicationId,
    ...
    it('should send email when upgrading from anon', done => {
        ...
        return rp
            ...
            .then(() => {
                ...
                expect(emailOptions.user.get('email')).toEqual(
                    'user@email.com'
                );
                done();
            })
                ...
                Parse.User.become(body.sessionToken).then(user => {
                    ...
                    return obj
                        ...
                        .then(() => {
                            ...
                            expect(body.sessionToken).not.toEqual(
                                user.getSessionToken()
                            );
                        })
                        .then(() => obj.fetch())
